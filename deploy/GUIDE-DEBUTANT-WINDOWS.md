...
| Les scripts `.ps1` refusent de s'exécuter (« n'est pas signé numériquement ») | fichiers marqués « vient d'Internet » (zip téléchargé) et éditeur des scripts pas encore approuvé sur ce PC | lancer `.\trust.cmd` une fois (répondre **Oui** à l'avertissement de sécurité ; `install.cmd` et `upgrade.cmd` le font déjà) — les scripts sont signés par « IRIS Station - Signature des scripts » ; sinon le `.cmd` du même nom, qui contourne la politique |
