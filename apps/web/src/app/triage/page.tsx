    ...
    <section className="flex flex-col gap-4 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {ORDER.map((c) => {
          const cfg = COLORS[c];
          return (
            <div key={c} className="carte flex items-center gap-4 p-4" style={{ borderLeft: `4px solid ${cfg.hex}` }}>
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg text-lg font-bold text-white" style={{ backgroundColor: cfg.hex }}>
                {totals[c]}
              </div>
              <div className="min-w-0">
                <div className="text-xs font-semibold text-gray-700 dark:text-rdia-100">{m.triage[cfg.key]}</div>
                <div className="text-[10px] text-gray-400 dark:text-rdia-400">{Math.round((totals[c] / totalVictims) * 100)}%</div>
              </div>
            </div>
          );
        })}
        ...
        <div className="flex items-center gap-3">
          {[
            ...
            <div key={stage.label} className="flex flex-1 items-center gap-3">
              <div className="flex-1 rounded-lg bg-gray-50 p-4 text-center dark:bg-rdia-800/50">
                <div className={`text-3xl font-bold tabular-nums ${stage.tint}`}>{stage.value}</div>
                <div className="mt-1 text-[10px] uppercase tracking-wider text-gray-400 dark:text-rdia-400">{stage.label}</div>
              </div>
              {i < arr.length - 1 && (
                <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-gray-300 dark:text-rdia-600 rtl:rotate-180">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {TRIAGE_ZONES.map((z) => {
            ...
            return (
              <div key={z.id} className="carte flex flex-col gap-3 p-5">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-bold text-rdia-600 dark:text-rdia-50">{z.name}</h4>
                  <span className="font-mono text-xs text-gray-400 dark:text-rdia-400">{zTotal} {m.triage.kpi_victims.toLowerCase()}</span>
                </div>
      ...
      {/* Victimes récentes */}
      <Table headers={[m.triage.col_tag, m.triage.col_category, m.common.zone, "Heure", m.triage.col_destination]}>
        {VICTIMS.map((v) => (
