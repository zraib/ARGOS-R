...
export function ChangePasswordScreen() {
  ...
  // 16 px sur mobile : sous cette taille, iOS zoome au focus et décale la page.
  const champCls = "input-champ text-base md:text-sm";

  return (
    <section
      // `min-h-dvh` : `100vh` déborde sous la barre d'adresse mobile. Centrage
      // vertical par `my-auto` sur la carte : sur écran court, son haut reste
      // atteignable au défilement (ce que `items-center` interdirait).
    ...
    >
      <div className="carte my-auto flex w-full max-w-sm animate-fade-in-up flex-col gap-5 p-5 sm:p-8">
        ...
        {sessionUser && (
          <div className="break-words rounded-lg bg-gray-50 px-3 py-2 text-center text-xs text-gray-500 dark:bg-rdia-900/40 dark:text-rdia-200">
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <input type="password" className={champCls} value={pass} onChange={(e) => { setPass(e.target.value); setError(null); }} onKeyDown={onKey} autoFocus />
          </div>
          <div>
            <label className={labelCls}>{m.users.cp_confirm}</label>
            <input type="password" className={champCls} value={confirm} onChange={(e) => { setConfirm(e.target.value); setError(null); }} onKeyDown={onKey} />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <button className="btn-primaire min-h-[44px] w-full text-sm" onClick={() => void submit()} disabled={busy}>{busy ? "…" : m.users.cp_submit}</button>
        </div>
      </div>
    </section>
  );
