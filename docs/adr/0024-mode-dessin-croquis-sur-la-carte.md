...
1. **Un croquis est une entité du domaine** (`Drawing` : nature `point` / `circle` / `polygon`,
   ...
   et dates). Persisté avec le domaine ; **tout le monde voit et dessine** (`map:view`, que
   tous les rôles des deux profils détiennent) ; **un croquis ne se modifie et ne se retire que
   par son auteur ou par le Super Administrateur** (règle portée par la route, 403 pour les
   autres — l'administrateur compris ; audité). La géométrie est vérifiée (400). Un événement
   temps réel `drawings` fait relire tous les postes.
...
4. **Le panneau « Dessin »** de la carte (bouton de la barre d'outils, onglet de la feuille
   mobile), ouvert à tous, porte les quatre outils, la liste des croquis, et la fiche du croquis
   sélectionné (nom, note, couleur, rayon, suppression confirmée) — en lecture seule, sans
   poignées ni étiquette déplaçable, quand le croquis est à quelqu'un d'autre. Les modifications rapides (une lettre du
...
- Dessiner ne dépend pas du mode édition de la carte : couper `map_edit` à un rôle ne lui retire
  pas le dessin ; couper `map` (la carte elle-même) le lui retire.
